...
/*
 *   STUDENT NAME      :  Ousmane Baricisse
...
public class NaturalNumber  {
	...
	public NaturalNumber plus( NaturalNumber  second) throws Exception{
        ...
        for(int i = 0;i<firstClone.coefficients.size();i++){
            sumNumber = firstClone.coefficients.get(i)+secondClone.coefficients.get(i)+carryNumber;
            carryNumber = sumNumber / this.base;
            sum.coefficients.add(sumNumber % this.base);
        }
        if(carryNumber>0)
            sum.coefficients.add(carryNumber);
	...
	public NaturalNumber times( NaturalNumber multiplicand) throws Exception{
		...
		int carry = 0;
        NaturalNumber singProduct = new NaturalNumber(this.base);
        NaturalNumber groupProduct = new NaturalNumber(this.base);
        
        //Go through every number of multiplicand to multiply
        for(int i = 0;i<multiplicand.coefficients.size();i++){
            //Multiply every number by the i number of the multiplicand
            groupProduct = new NaturalNumber(this.base);
            singProduct = new NaturalNumber(this.base);
            carry = 0;
            //Add 0s for the next group
            for(int y = 0;y<i;y++){
                groupProduct.coefficients.add(0);
            }
            for(int x = 0; x<this.coefficients.size();x++){
                //get individual product of each coefficient
                singProduct = singProduct.singleTimesHelper(multiplicand.coefficients.get(i),this.coefficients.get(x));
                //in case we have to carry
                singProduct = singProduct.plus(new NaturalNumber(carry,this.base));
                carry = 0;
                //if I need to carry something
                if(singProduct.coefficients.size()>1){
                    carry = singProduct.coefficients.getLast();
                }
                //Add base coefficient
                groupProduct.coefficients.add(singProduct.coefficients.getFirst());
                
            }
            //if we have to add one more coefficient
            if(carry>0){
                groupProduct.coefficients.add(carry);
            }
            //add with previous data
            product = product.plus(groupProduct);
        }
	...
	public NaturalNumber singleTimesHelper(Integer a, Integer b){
      NaturalNumber product;
      try {
          product = new NaturalNumber(0,this.base);
          for(int i = 0;i<b;i++){
              product = new NaturalNumber(a,this.base).plus(product);
          }
          return product;
      } catch (Exception e) {
          e.printStackTrace();
	...
	public NaturalNumber  minus(NaturalNumber second) throws Exception{
		...
		//Add 0s where necessary
		  int diff; // this is the difference between 2 coefficients
	        
	        int i = 0;
	        while (i<  first.coefficients.size()) {
	            
	            // if second has a coefficient at the given position then take the difference.
	            // if not then use it as 0, copy the coefficient
	            
	            if (i< second.coefficients.size()) {
	                
	                diff = first.coefficients.get(i).intValue() - second.coefficients.get(i).intValue();
	                ...
	                if (diff<0) {
	                    ...
	                    difference.coefficients.addLast(new Integer(diff + base));
	                }
	                
	                else 
	                    difference.coefficients.addLast(new Integer(diff));
	...
	public NaturalNumber divide( NaturalNumber  divisor ) throws Exception{
        ...
        while (remainder.compareTo(divisor)>=0) {
          int shift = 0;
          NaturalNumber shiftedDivisor = divisor.clone();
          
          while (remainder.compareTo(shiftedDivisor) >= 0) {
              shiftedDivisor.coefficients.addFirst( new Integer(0));
              shift ++;
          } //multiply the divisor by the base repeatedly while still keep it smaller/equal to the remainder
          
          shiftedDivisor.coefficients.removeFirst();
          shift --; //undo the last shift because I haven't found a numeric way to stop the loop right before it shifts too far
          
          int shiftCount = 0; 
          while (shiftedDivisor.helperTimesSingle(shiftCount + 1).compareTo(remainder) <= 0) {
              shiftCount ++;
          }
          
          remainder = remainder.minus(shiftedDivisor.helperTimesSingle(shiftCount)); //get the remaining remainder
          
          NaturalNumber quotientPart; 
          quotientPart = new NaturalNumber(shiftCount, this.base); //the quotient part from this loop
          
          quotient = quotient.plus(quotientPart.timesBaseToThePower(shift)); //build up the quotient 
	 ...
	 */
	  private NaturalNumber helperTimesSingle (int num) {
        
        int temp;
        int carry = 0;
        
        NaturalNumber productSingle = new NaturalNumber(this.base);
        for (int i=0; i<this.coefficients.size(); i++) {
            temp = this.coefficients.get(i)*num + carry;
            productSingle.coefficients.addLast(temp%base);
            carry = temp/base;
        }
        
        if (carry>0)
            productSingle.coefficients.addLast(carry);
